...
We can resize them using the general to get the same size
and use the align options to make the tidy

Change values to use % cacls and show the data labels

Add tooltips to visuals

You can add visuals from the toolbar as well
and can overlay one on top of another e.g. Card with a KPI behind a Donut
...
In order to get a rank we need to Provide a list of things to rank e.g. Country
and a value to rank. We have to use a calculate function get the sum
